...
flag = True
file_path = os.path.join(base_path, file_names[6])
print(f"\n{'=' * 60}")
print(f"📂 File: {file_names[6]}")
print(f"{'=' * 60}")
try:
    df = pd.read_csv(file_path)
    if(flag):
        specific_columns = {'record'}
        all_column_names = df.columns.tolist()
        columns_to_drop_by_index = [col_name for idx, col_name in enumerate(all_column_names) if idx >= 28 and idx <= 44]
        ...
        output_csv_name = 'my_team_yearly_stats_defense.csv'
    ...
    print(f"⚠️ Error reading {file_names[6]}: {e}")